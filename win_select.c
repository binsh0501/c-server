...
char *readFileAndReplacePlaceholder(const char *fname, const char *placeholder, const char *replacement);
    ...
    for (int i = 0; i < nTotalSockets; i++) {
      ...
      if (FD_ISSET(ptr->sock, &rset)) {
        ...
        if (strcmp(ptr->path, "/") == 0) {
          char *visitor_name = "정빈"; // 여기 수정해서 쓰면 됨
          buf = readFileAndReplacePlaceholder("login.html", "{이름}", visitor_name);
...
char *readFileAndReplacePlaceholder(const char *fname, const char *placeholder, const char *replacement) {
  ...
  size_t new_content_len = content_len - placeholder_len + replacement_len;
  char *new_content = (char *)malloc(new_content_len + 1);
  if (new_content == NULL) {
    free(content);
    perror("Memory allocation failed");
    return NULL;
  ...
  memcpy(new_content + (pos - content), replacement, replacement_len);
  strcpy(new_content + (pos - content) + replacement_len, pos + placeholder_len);
